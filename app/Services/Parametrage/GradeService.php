...
class GradeService
{
    ...
    {
        try {
            $response = $this->apiClient->get('grades', $filters);
        ...
        if (! $response->successful()) {
            return $this->emptyResult($response->json('message', 'Impossible de charger les grades.'));
        }
    ...
    public function create(array $data): array { return $this->save(fn () => $this->apiClient->post('grades', $data)); }
    public function update(int $id, array $data): array { return $this->save(fn () => $this->apiClient->put("grades/{$id}", $data)); }
    public function delete(int $id): array { return $this->save(fn () => $this->apiClient->delete("grades/{$id}")); }
