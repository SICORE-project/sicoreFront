  ...
  <section class="content-area">
    ...
    <form class="filter-panel rubrique-filters" method="GET" action="{{ route('parametres.rubriques-paie.index') }}" data-rubrique-filters>
      <div class="form-group rubrique-search">
        ...
        <input class="form-control" id="rubriqueSearch" name="search" type="search" value="{{ $filters['search'] ?? '' }}" placeholder="Code ou libellé…" autocomplete="off">
      </div>
    ...
    <section class="table-card" aria-labelledby="rubriqueListTitle">
      ...
      <div class="table-responsive">
        <table class="table">
          <thead><tr><th>Code</th><th>Libellé</th><th>Type</th><th>Périodicité</th>@if ($canManage)<th class="actions-cell">Actions</th>@endif</tr></thead>
          <tbody>
            ...
            @empty
              <tr><td colspan="{{ $canManage ? 5 : 4 }}" class="empty-message show">Aucune rubrique de paie trouvée.</td></tr>
            @endforelse
          </tbody>
        </table>
      </div>
    ...
    document.querySelectorAll('[data-rubrique-delete]').forEach(function (form) {
      form.addEventListener('submit', function (event) {
        if (form.dataset.confirmed !== 'true') {
          ...
          window.confirmAction(form.dataset.confirmMessage, function () {
