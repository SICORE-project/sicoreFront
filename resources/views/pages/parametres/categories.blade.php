...
<section class="content-area">
  ...
  <p class="breadcrumb">Paramétrage &gt; Catégories</p>
  <div class="categorie-toolbar">
    <form id="categorieFilterForm" method="GET" action="{{ route('parametres.categories.index') }}">
      <label class="sr-only" for="categorieSearch">Rechercher par libellé</label>
      <input class="form-control" id="categorieSearch" name="search" value="{{ request('search') }}" placeholder="Rechercher par libellé…">
    </form>
    <button class="btn-primary" type="button" data-modal-open="categorie-create-modal"><i class="fa-solid fa-plus" aria-hidden="true"></i> Nouvelle catégorie</button>
  </div>
  ...
  @empty<tr><td colspan="3" class="empty-message"><x-table-empty-state>Aucune catégorie trouvée.</x-table-empty-state></td></tr>@endforelse
  </tbody></table></div>@include('components.pagination', ['pagination' => $pagination])</section>
</section></main>
...
@push('styles')
<style>
  .categorie-toolbar { display: flex; align-items: center; gap: 20px; padding: 16px; border: 1px solid var(--border); border-radius: var(--radius-card); background: #fff; }
  #categorieFilterForm { flex: 1; min-width: 0; margin: 0; }
  #categorieSearch { width: 100%; }
  .categorie-toolbar > button { flex-shrink: 0; white-space: nowrap; }
  #categorie-create-modal .modal-dialog, #categorie-edit-modal .modal-dialog { width: calc(100% - 32px); max-width: 920px; }
  @media (max-width: 600px) {
    .categorie-toolbar { flex-wrap: wrap; }
    #categorieFilterForm { flex-basis: 100%; }
    .categorie-toolbar > button { width: 100%; justify-content: center; }
  }
</style>
@endpush
@push('scripts')
